...
items = ["(1) ATP", 
         ...
         "(5) Basket Team NBA 2022-23", 
         "(6) Basket Player NBA 2022"
]
...
if Sport == 'Basketball':
    ...
    if Team_vs_Player == 'Team':
        ...
        df_summaries = pd.read_excel('5_NBA_Team_Stats_Regular_Season_Team_Summaries.xlsx', sheet_name= 'PBC NBA 1947 - Team Summaries')
        df_totals = pd.read_excel('5_NBA_Team_Stats_Regular_Season_Team_Totals.xlsx', sheet_name= 'PBC NBA 1947 - Team Totals')
        ...
        with tabs[0]:
            ...
            st.pyplot() # Displaying plot in Streamlit
            st.markdown("**Figure 6**: Results from DICE. Representation of all the counterfactuals obtained. Provides the distribution of differences with a smooth representation of the data's probability density.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown("**Figure 7**: Results from DICE. Representation of all the counterfactuals obtained. Visual understanding of the entire dataset variations per feature as a all.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.pyplot() # Displaying plot in Streamlit
            st.markdown("**Figure 8**: Results from DICE. Representation of all the counterfactuals obtained. Provides the individual differences for each feature, with a clear view of the distribution of differences. Absolute values per feature.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[1]:
            ...
            st.markdown("**Figure 9**: Results from DICE. Representation of all the counterfactuals obtained. How the features would need to be altered in the counterfactual scenarios compared to the original data to achieve the desired outcomes predicted by the model.  \n - Positive values indicate an increase in a feature.  \n - Negative values indicate a decrease.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[2]:
            ...
            st.pyplot()
            st.markdown("**Figure 13**: Overview of the impact of each feature on the model output/predictions for a specific instance. So, the higher the SHAP Value mean, the higher its importance.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.pyplot()
            st.markdown("**Figure 15**: Summarizing the effects of all the features. Overview of which features are most important for a model by plotting the SHAP values of every feature for every sample. The plot below: \n - sorts features by the sum of SHAP value magnitudes over all samples; \n - uses SHAP values to show the distribution of the impacts each feature has on the model output. \n The color represents the feature value: \n - red high; \n - blue low.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.pyplot()  
            st.markdown("**Figure 19**: Take the mean absolute value of the SHAP values for each feature to get: \n - standard bar plot (produces stacked bars for multi-class outputs).")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[3]:
            # 5.2.1 Scatter Plot
            basketball_team_index = Basketball_team_list.index(Basketball_team_feature)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.scatter(shap_values[:, basketball_team_index])
            st.pyplot()
            ...
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Basketball_team_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Basketball_team_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.waterfall(shap_values[basketball_team_index])
            st.pyplot()
            st.markdown("**Figure 16**: Visualize the first prediction's explanation. Features each contributing to push the model output from the base value (X dataset) to the model output (final dataset). \n - Features pushing the prediction higher are shown in red. \n - Features pushing the prediction lower are in blue.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[4]:
            ...
            st.image("Rank_vs_Score_(5) Basket Team.png")
            ...
            unary_values = pd.read_csv("cs_rankqoi_unary_values_(5)_basket_team.csv")
            unary_values.set_index(unary_values.columns[0], inplace=True)
            unary_values_team = unary_values.loc[Team]
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            #fig, ax = plt.subplots()  
            values = xai.individual(unary_values_team.values)
            #xai.plot.waterfall(unary_values_team.values)
            xai.plot.waterfall(values)
            st.pyplot()
            st.markdown("**Figure 26**: Waterfall plot for the selected {Team}.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
    ...
    # Check if the user selects the type as Player
        ...
        # df
        df = pd.read_excel('6_NBA_Player_Stats_Regular_Season_2022_2023.xlsx', sheet_name= 'PBC 2022_23 NBA Player Stat')
        #st.write(df)
        df.info()
        ...
        with tabs[3]:
            # 5.2.1 Scatter Plot
            basketball_player_index = Basketball_player_list.index(Basketball_player_feature)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.scatter(shap_values[:, basketball_player_index])
            st.pyplot()
            ...
            shap.partial_dependence_plot(
                ...
                model_expected_value=True, feature_expected_value=True) 
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Basketball_player_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Basketball_player_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.waterfall(shap_values[basketball_player_index])
            st.pyplot()
            st.markdown("**Figure 16**: Visualize the first prediction's explanation. Features each contributing to push the model output from the base value (X dataset) to the model output (final dataset). \n - Features pushing the prediction higher are shown in red. \n - Features pushing the prediction lower are in blue.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[4]:
            ...
            import os
            st.markdown(f"<h4 style='text-align: center;'>SHARP: Rank vs Score</h1>", unsafe_allow_html=True)
            st.image("Rank_vs_Score_(6) Basket Player.png")
            st.markdown("**Figure 22**: Relationship between Score and Rank.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>Table: Unary values used in our analysis</h1>", unsafe_allow_html=True)
            unary_values = pd.read_csv("cs_rankqoi_unary_values_(6)_basket_player.csv")
            unary_values.set_index(unary_values.columns[0], inplace=True)
            unary_values_player = unary_values.loc[Player]
            st.write(unary_values)
            st.markdown("**Figure 25**: Representation of all Unary Values. This aggregates all data used in our research.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)

            #basketball_team_index = Basketball_player_list.index(Basketball_player_feature)
            #shap.plots.waterfall(shap_values[basketball_player_index])
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            #fig, ax = plt.subplots()  
            values = xai.individual(unary_values_player.values)
            #xai.plot.waterfall(unary_values_team.values)
            xai.plot.waterfall(values)
            st.pyplot()
            st.markdown("**Figure 26**: Waterfall plot for the selected {Player}.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
    ...
    if Team_vs_Player == 'Team':
        ...
        with tabs[3]:
            # 5.2.1 Scatter Plot
            Football_team_index = Football_team_list.index(Football_team_feature)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.scatter(shap_values[:, Football_team_index])
            st.pyplot()
            ...
            shap.partial_dependence_plot(
                ...
                model_expected_value=True, feature_expected_value=True) 
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Football_team_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Football_team_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>SHAP Waterfall Plot for {Team}</h1>", unsafe_allow_html=True)
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.waterfall(shap_values[Football_team_index])
            st.pyplot()
            st.markdown("**Figure 16**: Visualize the first prediction's explanation. Features each contributing to push the model output from the base value (X dataset) to the model output (final dataset). \n - Features pushing the prediction higher are shown in red. \n - Features pushing the prediction lower are in blue.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
    ...
    # Check if the user selects the type as Player
        ...
        #Player = st.sidebar.selectbox('Select the Player:', ('Cristiano Ronaldo dos Santos Aveiro', 'Messi', 'Neymar'))
        #st.sidebar.write(f"You selected {Player} as the player.")
        #st.write('You selected Football. Here is some specific information about it.')
        #st.write('You selected {Player}. Now, we will present the base of this project.')
        
        # df
        df = pd.read_excel('4_Football_Player_FIFA_2022.xlsx', sheet_name= 'PBC players_22')
        #st.write(df)
        df.info()
        ...
        with tabs[3]:
            # 5.2.1 Scatter Plot
            Football_player_index = Football_player_list.index(Football_player_feature)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.scatter(shap_values[:, Football_player_index])
            st.pyplot()
            ...
            shap.partial_dependence_plot(
                ...
                model_expected_value=True, feature_expected_value=True) 
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Football_player_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Football_player_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>SHAP Waterfall Plot for {Player}</h1>", unsafe_allow_html=True)
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.waterfall(shap_values[Football_player_index])
            st.pyplot()
            st.markdown("**Figure 16**: Visualize the first prediction's explanation. Features each contributing to push the model output from the base value (X dataset) to the model output (final dataset). \n - Features pushing the prediction higher are shown in red. \n - Features pushing the prediction lower are in blue.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
    ...
    if Male_vs_Female == 'Male':
        ...
        # df_serve
        df_serve = pd.read_excel('1_ATP_info.xlsx', sheet_name= 'Serve 2022')
        #st.write(df_serve)
        
        # df_return
        df_return = pd.read_excel('1_ATP_info.xlsx', sheet_name= 'Return 2022')
        #st.write(df_return)
        
        # df_under_pressure
        df_underpressure = pd.read_excel('1_ATP_info.xlsx', sheet_name= 'UnderPressure 2022')
        ...
        with tabs[3]:
            # 5.2.1 Scatter Plot
            Tennis_male_index = Tennis_male_list.index(Tennis_male_feature)
            ...
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.scatter(shap_values[:, Tennis_male_index])
            st.pyplot()
            ...
            shap.partial_dependence_plot(
                ...
                model_expected_value=True, feature_expected_value=True) 
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Tennis_male_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Tennis_male_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>SHAP Waterfall Plot for {Player}</h1>", unsafe_allow_html=True)
            st.set_option('deprecation.showPyplotGlobalUse', False)
            shap.plots.waterfall(shap_values[Tennis_male_index])
            st.pyplot()
            st.markdown("**Figure 16**: Visualize the first prediction's explanation. Features each contributing to push the model output from the base value (X dataset) to the model output (final dataset). \n - Features pushing the prediction higher are shown in red. \n - Features pushing the prediction lower are in blue.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[4]:
            ...
            import os
            st.markdown(f"<h4 style='text-align: center;'>SHARP: Rank vs Score</h1>", unsafe_allow_html=True)
            st.image("Rank_vs_Score_(1) ATP.png")
            st.markdown("**Figure 22**: Relationship between Score and Rank.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>Table: Unary values used in our analysis</h1>", unsafe_allow_html=True)
            unary_values = pd.read_csv("cs_rankqoi_unary_values_(1)_atp.csv")
            unary_values.set_index(unary_values.columns[0], inplace=True)
            unary_values_player = unary_values.loc[Player]
            st.write(unary_values)
            st.markdown("**Figure 25**: Representation of all Unary Values. This aggregates all data used in our research.")
    ...
    # Check if the user selects the Female Player
        ...
        # df_serve
        df_serve = pd.read_excel('2_WTA.xlsx', sheet_name= 'Servers Stats 2023')
        #st.write(df_serve)
        
        # df_return
        df_return = pd.read_excel('2_WTA.xlsx', sheet_name= 'Return Stats 2023')
        #st.write(df_return)
        ...
        with tabs[3]:
            ...
            shap.partial_dependence_plot(
                ...
                model_expected_value=True, feature_expected_value=True) 
            st.pyplot()
            st.markdown(f"**Figure 21**: Visualization of the model's dependence on the feature {Tennis_female_feature_full_name}, now in the new original feature space (X).  It explains how the SHAP values of a particular feature vary across a dataset and how changes in the values of the first feature impact the model's predictions for various data points. \n - x-axis represents the SHAP values for the {Tennis_female_feature} feature. \n - y-axis represents the variation per player.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
        ...
        with tabs[4]:
            ...
            import os
            st.markdown(f"<h4 style='text-align: center;'>SHARP: Rank vs Score</h1>", unsafe_allow_html=True)
            st.image("Rank_vs_Score_(2) WTA.png")
            st.markdown("**Figure 22**: Relationship between Score and Rank.")
            st.write("<div style='height: 150px;'></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h4 style='text-align: center;'>Table: Unary values used in our analysis</h1>", unsafe_allow_html=True)
            unary_values = pd.read_csv("cs_rankqoi_unary_values_(2)_wta.csv")
            unary_values.set_index(unary_values.columns[0], inplace=True)
        ...
        with tabs[5]:
            ...
            st.markdown(f"**Figure 29**: Graphic representation for {Player} needs to must improve to get to decil.....")
